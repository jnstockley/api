...
images = ["adguard/adguardhome", "louislam/dockge", "jc21/nginx-proxy-manager",
          "portainer/portainer-ce", "louislam/uptime-kuma", "vaultwarden/server"]
...
def get_latest_docker_image(image: str) -> str | None:
    ...
    if response.status_code == 200:
        for docker_image in response.json()['results']:
            if 'digest' in docker_image and 'name' in docker_image and docker_image['digest'] == digest:
