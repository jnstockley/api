...
router = APIRouter(
    prefix='/docker',
    dependencies=[Security(get_token_header)]
)
...
async def get_latest_docker_version(docker_image: str = ""):
    if docker_image.strip() == "":
        raise HTTPException(status_code=422, detail='Missing docker_image query parameter')
    ...
    if latest_version is None:
        raise HTTPException(status_code=404, detail=f'{docker_image} not found in Docker Hub')
    newer_version = latest_version != current_version
    return {'image': f'{image}:{latest_version}', 'latest_image_version': latest_version,
